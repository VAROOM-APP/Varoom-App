...
const getMarkerIcon = (eventType, isSelected) => {
  ...
  const color = isSelected ? '#ffffff' : (colors[eventType] || colors.default);
  const emoji = emojis[eventType] || emojis.default;
  const size = isSelected ? 50 : 40;
  const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + (size + 10) + '" viewBox="0 0 40 50"><path d="M20 0C9 0 0 9 0 20c0 15 20 30 20 30S40 35 40 20C40 9 31 0 20 0z" fill="' + color + '" stroke="#e63946" stroke-width="2"/><circle cx="20" cy="20" r="13" fill="white"/><text x="20" y="26" text-anchor="middle" font-size="16" fill="black">' + emoji + '</text></svg>';
...
function MapView({ events, selectedEvent, setSelectedEvent, mapExpanded, setMapExpanded, mapFullScreen, setMapFullScreen, getCalendarUrl, mapsLoaded, filterLocation, distanceEnabled, distanceMiles }) {
  ...
  const mapHeightPx = mapFullScreen ? windowHeight : Math.floor(windowHeight * 0.6);
        ...
        <div style={{ width: '100%', height: mapHeightPx + 'px', position: 'relative' }}>
          ...
          >
            {mapsLoaded && events.map(event => (
              <Marker
                ...
                title={event.title}
                icon={{
                  url: getMarkerIcon(event.event_type, selectedEvent?.id === event.id),
                  scaledSize: selectedEvent?.id === event.id
                    ? new window.google.maps.Size(50, 60)
                    : new window.google.maps.Size(40, 50),
                  anchor: selectedEvent?.id === event.id
          ...
          {selectedEvent && (
            <div className="map-bottom-sheet" onClick={e => e.stopPropagation()}>
              <div className="map-bottom-sheet-handle" onClick={() => setSelectedEvent(null)} />
              <div className="map-bottom-sheet-content">
                ...
                <div className="sidebar-meta">
                  ...
                  {selectedEvent.location_name && <p>📍 {selectedEvent.location_name}</p>}
                ...
                <div className="map-bottom-sheet-actions">
                  <a href={'https://www.google.com/maps/dir/?api=1&destination=' + selectedEvent.latitude + ',' + selectedEvent.longitude} target="_blank" rel="noreferrer" className="sheet-action-btn directions-btn">🗺 Get Directions</a>
                  <a href={getCalendarUrl(selectedEvent)} target="_blank" rel="noreferrer" className="sheet-action-btn calendar-btn">📅 Add to Calendar</a>
                  {selectedEvent.external_link && <a href={selectedEvent.external_link} target="_blank" rel="noreferrer" className="sheet-action-btn tickets-btn">🎟 More Info / Tickets</a>}
                </div>
              </div>
            </div>
          )}
